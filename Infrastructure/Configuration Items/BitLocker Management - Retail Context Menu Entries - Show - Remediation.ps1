        ...
        foreach ($Item in $Temp_Registry_Entries) {
            ...
            try {
                ...
                if ($Temp_Registry_Entries_Return -notin "",$null) {
                    # Remove Found Registry Properties
                        ...
                        Remove-ItemProperty -Path $Item.Value.Path -Name $Item.Value.Name -ErrorAction $Item.Value.ErrorAction | Out-Null
                        $Meta_Remediation_Action = "Removed"
                }
                else {
                    # Do Nothing, Property Not Found
