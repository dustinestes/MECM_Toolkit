...
#--------------------------------------------------------------------------------------------
# Start-Transcript -Path "C:\VividRock\MECM Toolkit\Logs\Task Sequences\Windows - NetJoinLegacyAccountReuse.log" -ErrorAction SilentlyContinue
...
#Region Header
  ...
  Write-Host "------------------------------------------------------------------------------"
  Write-Host "  MECM Toolkit - Task Sequences - Windows - NetJoinLegacyAccountReuse"
  Write-Host "------------------------------------------------------------------------------"
  Write-Host "    Author:     Dustin Estes"
  Write-Host "    Company:    VividRock"
  Write-Host "    Date:       2024-0-29"
  Write-Host "    Copyright:  VividRock LLC - All Rights Reserved"
  Write-Host "    Purpose:    Enable or disable the the reuse of an AD account during domain join."
  Write-Host "    Links:      https://support.microsoft.com/en-us/topic/kb5020276-netjoin-domain-join-hardening-changes-2b65a0f3-1f4c-42ef-ac0f-1caaf421baf8"
...
#Region Variables
  ...
  # Hashtables
    $Hashtable_Offline = @{
      "FilePath"      = "$($Param_Drive)Windows\System32\config\System"
  ...
  # Registry
    $Registry_Enable  = @{
      "Path"          = ""
      "PathWin"       = "HKLM:\SYSTEM\CurrentControlSet\Control\Lsa"
      "PathWinPE"     = "HKLM:\Offline\ControlSet001\Control\LSA"
...
#Region Execution
  ...
  # Load Offline Registry Hive
	...
	# Disable NetJoinLegacyAccountReuse
    if ($Param_Enabled -eq "False") {
      try {
		    Write-Host "    - Disable NetJoinLegacyAccountReuse"
