            ...
            try {
                # Perform Multiple Attempts to Allow for Replication Delays in Active Directory
                    ...
                    do {
                        $Counter_Enroll_Attempts ++
                        Write-Host "        Attempt: $($Counter_Enroll_Attempts)"
                        $Result_Enroll = Get-Certificate -Template $Param_Certificate_Template -Url $Param_Certificate_ServerURL -CertStoreLocation $Param_Certificate_TargetStore -ErrorAction SilentlyContinue
