...
param (
    [string]$EXEPath,                 # '%vr_USMT_ContentPath01%\amd64\scanstate.exe'
...
#Region Header
    ...
    Write-Host "------------------------------------------------------------------------------"
    Write-Host "  Task Sequence Toolkit - USMT - ScanState"
    Write-Host "------------------------------------------------------------------------------"
    ...
    Write-Host "                for the ScanState tool."
    Write-Host "    Links:      https://learn.microsoft.com/en-us/windows/deployment/usmt/usmt-scanstate-syntax"
    Write-Host "------------------------------------------------------------------------------"
    Write-Host ""
...
#Region Variables
    ...
    # Paths
        $Path_USMT_ScanStateEXE     = $EXEPath
    ...
    # Output to Log
        Write-Host "    - Paths"
        Write-Host "        ScanState EXE: $($Path_USMT_ScanStateEXE)"
...
#Region Validation
    ...
    # ScanState EXE Exists
        Write-Host "    - ScanState EXE Exists"

        try {
            Write-Host "        Path: $($Path_USMT_ScanStateEXE)"
            if ((Test-Path -Path $Path_USMT_ScanStateEXE -ErrorAction Stop) -eq $true) {
                Write-Host "        Success"
            }
            else {
                Write-Host "        Failure"
...
#Region Execution
    ...
    # Construct Installation String
        ...
        try {
            ...
            $Temp_Install_Parameters    = "/o /localonly /nocompress /efs:copyraw /v:5 /c /uel:60"
            $Temp_Install_Logging       = "/l:`"$($Path_USMT_Log)\scanstate.log`""
            $Temp_Install_Progress      = "/progress:`"$($Path_USMT_Log)\scanstateprogress.log`""
            ...
            Write-Host "        Command: $($Path_USMT_ScanStateEXE + `" `" + $Temp_Install_ArgumentList)"
            Write-Host "        Success"
        }
        catch {
    ...
    # Run USMT ScanState
        Write-Host "    - Run USMT ScanState"

        try {
            # Execute
                $Result_Scanstate = Invoke-Expression -Command "$($Path_USMT_ScanStateEXE) $Temp_Install_ArgumentList"

            # Cleanup Output
                # Remove Empty Elements in Array
                    $Result_Scanstate_Clean = $Result_Scanstate | Where-Object -FilterScript {$_ -notin "",$null," "}

            # Evaluate Output
                if ($LASTEXITCODE -ne 0) {
                    # Output
                        Write-Host "        Error: $($LASTEXITCODE)"
                        Write-Host "        Message: $($Result_Scanstate_Clean[$($Result_Scanstate_Clean.Count - 3)])"
                        Exit $LASTEXITCODE
                }
                else {
                    Write-Host "        Success"
